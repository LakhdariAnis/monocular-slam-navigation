...
const ANOMALY_TYPES = [{
  ...
  key: "slam_low_feature",
  label: "Low Feature Room",
  color: "#facc15"
}];
const PHASE_COLORS = {
  departing: {
...
function PositionPathChart({
  ...
  const renderDot = props => {
    ...
    const dotColor = isLast ? degraded ? degradedColor : lastColor : degraded ? degradedColor : normalColor;
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    data: points,
    line: {
      stroke: slamRateStatus && slamRateStatus !== "ok" ? "#facc15" : "#7dd3fc",
      strokeWidth: 2,
...
function SlamRatePanel() {
  ...
  useEffect(() => {
    let alive = true;
    const poll = () => {
      fetch(`${API_BASE}/live/slam_rate`).then(r => r.json()).then(d => {
        if (alive) setRateData(d);
      }).catch(() => {});
    };
    poll();
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "font-label text-xs text-on-surface-variant mb-1 flex items-center gap-2"
  }, /*#__PURE__*/React.createElement("span", {
    className: "material-symbols-outlined text-sm"
  }, "speed"), "SLAM Publish Rate"), /*#__PURE__*/React.createElement("div", {
  ...
  }, "msgs/sec"), /*#__PURE__*/React.createElement("span", {
    ...
    if (toggles.motor_stall && !prevMotorRef.current) {
      fetch(`${API_BASE}/api/publish`, {
        ...
        body: JSON.stringify({
          topic: "car/mock/motor_stall_params",
          payload: {
            severity: parseFloat(motorStallSeverity.toFixed(2))
          }
        })
      }).catch(() => {});
    }
    ...
    fetch(`${API_BASE}/api/publish`, {
      ...
      body: JSON.stringify({
        ...
        payload: {
          ...
          recover: parseFloat(recover.toFixed(4))
        }
      })
    }).catch(() => {});
  };
  ...
  const renderSubControls = key => {
    switch (key) {
      case "motor_stall":
        ...
        }, motorStallSeverity.toFixed(2))), /*#__PURE__*/React.createElement("input", {
          type: "range",
          min: "0",
          max: "1",
          step: "0.01",
        ...
        }, /*#__PURE__*/React.createElement("span", null, "0.0 (arc)"), /*#__PURE__*/React.createElement("span", null, "1.0 (freeze)")));
      case "slam_low_feature":
        return /*#__PURE__*/React.createElement("div", {
          ...
          className: "font-label text-xs text-on-surface-variant"
        }, "Room Quality"), /*#__PURE__*/React.createElement("span", {
          className: "font-display text-sm",
          style: {
            color: "#facc15"
          }
        }, slamQuality, "%")), /*#__PURE__*/React.createElement("input", {
          type: "range",
          min: "0",
          max: "100",
          value: slamQuality,
...
function App() {
  ...
  useEffect(() => {
    ...
    const poll = () => {
      fetch(`${API_BASE}/live/slam_rate`).then(r => r.json()).then(d => {
        if (alive) setSlamRateStatus(d.status);
      }).catch(() => {});
    };
    poll();
    const id = setInterval(poll, 500);
